...
export function activate(context: vscode.ExtensionContext) {
	...
	function getEditorAndSelection(): { editor: vscode.TextEditor, startLine: number, endLine: number } | undefined {
	...
	function createIndentCommand(level: number) {
		return vscode.commands.registerCommand(`indent-selection.indentBlock${level}`, async () => {
			...
			for (let i = startLine; i <= endLine; i++) {
				...
				const indentationSize = editor.options.tabSize as number;
				let indentText = '';
	...
	function createOutdentCommand(level: number) {
				...
				if (useSpaces) {
					...
					if (spacesToRemove > 0) {
						edit.delete(editor.document.uri, new vscode.Range(
							...
							new vscode.Position(i, spacesToRemove)
						));
					}
				} else {
					...
					if (tabsToRemove > 0) {
						edit.delete(editor.document.uri, new vscode.Range(
							new vscode.Position(i, 0),
							new vscode.Position(i, tabsToRemove)
						));
					}
				}
			}
